...
from home_ai.common.models import ChatRequest, ChatResponse, IoTResult
...
_iot_controller: Optional[IoTController] = None
...
async def chat(request: ChatRequestAPI):
    ...
    result = ChatResponseAPI(
        text=response.text,
        commands_executed=[cmd.model_dump() for cmd in response.commands],
...
async def control_light(
    ...
    controller: IoTController = Depends(get_iot_controller)
):
...
async def control_alarm(
    ...
    result = await controller.control_alarm(
        action=request.action,
        time=request.time,
        label=request.label
    )
